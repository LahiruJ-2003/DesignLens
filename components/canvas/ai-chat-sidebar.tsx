...
//   // Generate design context for AI
//   const generateDesignContext = () => {
...
//   .map(
//     (el) =>
//       `- ${el.type}: position(${Math.round(el.x)}, ${Math.round(el.y)}), size(${Math.round(el.width)}x${Math.round(el.height)}), fill: ${el.fill || "none"}`
//   )
//   .join("\n")}
// ${elements.length > 10 ? `... and ${elements.length - 10} more elements` : ""}`;
//   };
...
//           messages,
//           id,
//           designContext: generateDesignContext(),
//         },
//       }),
//     }),
//   });
